...
process metatraits_speci_call {
	...
	output:
	tuple val(speci), path("${speci}/metatraits/${speci}.traits_from_speci.json"), emit: metatraits

	script:
	"""
	mkdir -p ${speci}/metatraits/

	metatraits_comm.py --speci ${speci} -o ${speci}/metatraits/${speci}.traits_from_speci.json
	"""
